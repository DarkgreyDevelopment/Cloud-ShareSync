...
namespace Cloud_ShareSync.GUI.Types {
    internal class ConfigureSyncTab : StackPanel, IConfigurationTab {
        ...
        private readonly TextBlock _syncFolderHeader = new( ) {
            Text = "Sync Folder: ",
        ...
        private readonly TextBlock _syncFolderTxt = new( ) {
            HorizontalAlignment = HorizontalAlignment.Left,
            VerticalAlignment = VerticalAlignment.Center,
            FontSize = 16
        };

        private readonly TextBlock _noSyncFolderTxt = new( ) {
            Name = "NoSyncFolderTxt",
            Text = "Sync folder doesn't exist!",
        ...
        private readonly Button _updateSyncFolderButton = new( ) {
        ...
        private readonly TextBlock _workDirHeader = new( ) {
        ...
        private readonly TextBlock _workDirTxt = new( ) {
            HorizontalAlignment = HorizontalAlignment.Left,
            VerticalAlignment = VerticalAlignment.Center,
            FontSize = 16
        };

        private readonly Button _updateWorkDirButton = new( ) {
            Name = "UpdateConfigPath",
            Margin = Thickness.Parse( "10,5,5,15" ),
            HorizontalAlignment = HorizontalAlignment.Left,
            HorizontalContentAlignment = HorizontalAlignment.Center,
            VerticalAlignment = VerticalAlignment.Bottom,
            IsEnabled = true,
        ...
        private void ConfigureWindowContent( SyncConfig config ) {
            AddWrapPanelOne( config.SyncFolder );              // _syncFolderHeader, _syncFolderTxt
            AddWrapPanelTwo( config.Recurse );                 // _updateSyncFolderButton, _noSyncFolderTxt, _recurseSyncFolderCheckBox
            AddExcludePathsSection( config.ExcludePaths );     // _excludePathsHeader, _excludePathsTxtBox
            AddWrapPanelThree( config.WorkingDirectory );      // _workDirHeader, _workDirTxt
            AddUpdateWorkDirButton( );                         // _updateWorkDirButton
        ...
        #region SyncFolder Methods

        private void SyncFolderExistsWarning( string syncFolder ) {
            if (Directory.Exists( syncFolder )) {
                _noSyncFolderTxt.IsVisible = false;
                _syncFolderTxt.Foreground = Brushes.Black;
                _syncFolderTxt.Background = Brushes.White;
            } else {
                _syncFolderTxt.Foreground = Brushes.White;
                _syncFolderTxt.Background = Brushes.Red;
                _noSyncFolderTxt.IsVisible = true;
            }
        }

        private void AddWrapPanelOne( string syncFolder ) {
            WrapPanel wrap = new( ) {
                Margin = Thickness.Parse( "5,15,5,0" )
            };
            wrap.Children.Add( _syncFolderHeader );
            wrap.Children.Add( _syncFolderTxt );
            _syncFolderTxt.Text = syncFolder;
            SyncFolderExistsWarning( syncFolder );
            Children.Add( wrap );
        }

        private void AddWrapPanelTwo( bool recurse ) {
            WrapPanel wrap2 = new( ) {
                Margin = Thickness.Parse( "5,0,5,15" )
            };
            wrap2.Children.Add( _updateSyncFolderButton );
            _updateSyncFolderButton.Click += ChangeSyncFolderHandler;
            wrap2.Children.Add( _noSyncFolderTxt );
            NewRecurseCheckbox( recurse, wrap2 );
            Children.Add( wrap2 );
        }

        private void NewRecurseCheckbox( bool recurse, WrapPanel wrap2 ) {
            _recurseSyncFolderCheckBox.IsChecked = recurse;
            wrap2.Children.Add( _recurseSyncFolderCheckBox );
        }

        #endregion SyncFolder Methods
        ...
        #region WorkingDirectory Methods

        private void AddWrapPanelThree( string workingDirectory ) {
            WrapPanel wrap3 = new( ) {
                Margin = Thickness.Parse( "5,15,5,0" )
            };
            wrap3.Children.Add( _workDirHeader );
            wrap3.Children.Add( _workDirTxt );
            _workDirTxt.Text = workingDirectory;
            Children.Add( wrap3 );
        }

        private void AddUpdateWorkDirButton( ) {
            _updateWorkDirButton.Click += ChangeWorkDirHandler;
            Children.Add( _updateWorkDirButton );
        }

        #endregion WorkingDirectory Methods
        ...
        private SyncConfig GetSyncConfig( ) {
            ...
            config.SyncFolder = _syncFolderTxt.Text;
            config.ExcludePaths = _excludePathsTxtBox.Text.Split( '\n' );
            config.WorkingDirectory = _workDirTxt.Text;
        ...
        public async void ChangeSyncFolderHandler( object? sender, RoutedEventArgs e ) {
            ...
            try {
                ...
                if (result?.Length > 0) {
                    _syncFolderTxt.Text = result;
                    SyncFolderExistsWarning( result );
                }
            } catch (Exception ex) {
        ...
        public async void ChangeWorkDirHandler( object? sender, RoutedEventArgs e ) {
            ...
            try {
                ...
                string? result = await dialog.ShowAsync( _parentWindow );
                if (result?.Length > 0) { _workDirTxt.Text = result; }
            } catch (Exception ex) {
                await new MessageBox(
